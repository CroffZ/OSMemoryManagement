...
PRIVATE int esc_start;      /* Esc Status	 */
PRIVATE int	tab;			/* Tab key		 */
...
/*======================================================================*
                            keyboard_handler
 *----------------------------------------------------------------------*
  处理输入中断，将输入的字符存入缓存区，若缓存区已满则直接丢弃
 *======================================================================*/
...
PUBLIC void keyboard_read(TTY* p_tty)
{
	...
	u32	key = 0;/* 用一个int来表示一个键。比如，如果 Home 被按下，
	...
	if(kb_in.count > 0){
		...
		if ((key != PAUSEBREAK) && (key != PRINTSCREEN)) {
			...
			case ESC:
				if (make) {
					esc_start=!esc_start;
					//在此处需要添加一个方法，用来在第二次esc时清除上一次输入，and turn back the color
				}
				break;
			case TAB:
